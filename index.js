            ...
            for (let item of itemsToSend) {
                ...
                getCondition().then((state) => {
                    const embed = new Discord.MessageEmbed()
                        ...
                        .setFooter({ text: `Créer par Luigi | ID : ${sub.id} | ${item.brand_title}` })
                        .addField('Prix', item.price + "0 €", true)
                        .addField('Vendeur', item.user.login || 'vide', true)
                        .addField('Taille', item.size_title || 'vide', true)
                        .addField('Condition', state || 'vide', true)
                        .addField('Échange ?', swap, true)
...
client.on('ready', () => {
    ...
    client.user.setActivity(`Recherche les meilleurs articles`);
});
